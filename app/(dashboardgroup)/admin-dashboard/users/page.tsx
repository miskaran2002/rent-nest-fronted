...
import { Search, ShieldAlert, ShieldCheck, Users, Clock } from 'lucide-react';
...
export default function AdminUsersPage() {
  ...
  // সার্চ এবং পেজিনেশন স্টেটস
  const [searchTerm, setSearchTerm] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 5; // প্রতি পেজে ৫ জন ইউজার প্রদর্শিত হবে
  ...
  // সার্চ বক্সে কিছু লিখলে কারেন্ট পেজ ১ নম্বরে রিসেট করার ইফেক্ট (UX Optimization)
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm]);
  ...
  // ১. সার্চ ফিল্টারিং লজিক
  const filteredUsers = users.filter((u) =>
    u.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    u.email.toLowerCase().includes(searchTerm.toLowerCase())
  );

  // ২. ⚠️ অ্যাসাইনমেন্টের রিকোয়ারমেন্ট অনুযায়ী ডায়নামিক পেজিনেশন লজিক (Slicing data)
    ...
    <div className="space-y-6 select-none">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 border border-zinc-900 rounded-xl bg-zinc-900/30 text-white placeholder-zinc-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-xs transition-all"
          />
        </div>
      </div>
      ...
      {paginatedUsers.length > 0 ? (
        <motion.div
          ...
          className="overflow-hidden rounded-2xl border border-zinc-900 bg-zinc-900/10 backdrop-blur-md shadow-xl"
        >
          {/* ইউজার টেবিল */}
          ...
          {/* ⚠️ গ্লোয়িং ড্যাশবোর্ড পেজিনেশন বার (Pagination Controls) */}
          {totalPages > 1 && (
            <div className="flex items-center justify-between p-4 border-t border-zinc-900 bg-zinc-950/40 text-xs">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-1.5 border border-zinc-800 bg-zinc-900/40 hover:bg-zinc-900 rounded-lg text-zinc-300 hover:text-white transition-all disabled:opacity-30 disabled:pointer-events-none cursor-pointer font-semibold"
                >
                  Previous
                </button>
                <button
